...
# Initializations
$test = Test::Cmd->new(prog    => "$iBackupCheck::SCRIPT",
                       workdir => '',
                       interpreter => 'perl',
                       verbose => $DEBUG
                      );
...
$test->preserve();
$conf      = iBackupCheck::Config->new();
$conf->verbose($DEBUG);
$conf_file = $conf->filename();

# Let's go
do_test_on_configfile($iBackupCheck::ExitCodes{'ibackup::Exception::CommandLineFormat'},
...
do_test_on_configfile($iBackupCheck::ExitCodes{'ibackup::Exception::ConfigFileFormat'},
                      'empty configuration file',
                      "-f $conf_file");

#$conf->add_param_to_section('global', 'src_rootdir', "$iBackupCheck::TESTDIR/");
$conf->initialize();
$conf->write();
do_test_on_configfile($iBackupCheck::ExitCodes{'ibackup::Exception::ConfigFileFormat'},
...
$conf->add_param_to_section('global', 'src_rootdir', "$iBackupCheck::TMPDIR/__hopefully_unexisting_dir__");
$conf->write();
do_test_on_configfile($iBackupCheck::ExitCodes{'ibackup::Exception::DirectoryNotFound'},
                      'non-existing source directory',
                      "-f $conf_file");

$conf->add_param_to_section('global', 'src_rootdir',  "$iBackupCheck::TESTDIR/t/data");
...
$conf->write();
do_test_on_configfile($iBackupCheck::ExitCodes{'ibackup::Exception::UnsupportedFeature'},
